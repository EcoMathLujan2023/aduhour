...
# Ejercicio asumiendo que son bacterias que crecen en un medio, hacer un repique 
# 
#
...
c4 = crec_exp(0.1,1.0,500,0.01,20.0)
plot(c4)

##
# Crecimiento exponencial estocástico, se calcula tiempo que transcurre 
# hasta un evento
# Suposiciones en un tiempo h sucede 1 solo evento
# Tomamos como evento a la reproduccion de un individuo 
# El tiempo que tarda en pasar un evento discreto se simula con una distribucion
# exponencial.
...
function crec_exp_sto_repique(λ,N₀,tfinal,trepique=0.0)    
    pop = [N₀]                        # Vector para la poblacion
    ts  = [0.0]                       # Vector de valores de tiempo
    i = 1                             # variable indice
    t = 0.0                           # variable auxiliar de tiempo
...
sr1 = crec_exp_sto_repique(0.1,1.0,100,5)
